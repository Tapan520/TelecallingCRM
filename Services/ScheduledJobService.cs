...
public class ScheduledJobService
{
    ...
    // ?????????????????????????????????????????????????????????????????????????
    // 6. Purge read notifications older than 30 days (daily 02:00)
    // ?????????????????????????????????????????????????????????????????????????
    [AutomaticRetry(Attempts = 1)]
    public async Task CleanupOldNotificationsAsync()
    {
        ...
        _logger.LogInformation("CleanupNotifications: removed {Count} old read notification(s).", old.Count);
    }
    ...
    public static void RegisterRecurringJobs()
    {
        ...
        RecurringJob.AddOrUpdate<ScheduledJobService>(
            ...
            s => s.CleanupOldNotificationsAsync(),
            "0 2 * * *"); // 02:00 UTC daily
    }
}
