...
// Railway injects PORT env var; bind to it so the health-check and reverse-proxy work
...
builder.Services.AddDbContext<AppDbContext>((sp, options) =>
{
    ...
    var interceptor = sp.GetService<TelecallingCRM.Data.AuditSaveChangesInterceptor>();
...
// ?? Audit Interceptor ???????????????????????????????????????????????????????
builder.Services.AddScoped<TelecallingCRM.Data.AuditSaveChangesInterceptor>();
...
builder.Services.AddOutputCache(options =>
{
    ...
    options.AddPolicy("leaderboard", policy =>
        policy.Expire(TimeSpan.FromSeconds(120)).Tag("leaderboard"));
    options.AddPolicy("reports", policy =>
        policy.Expire(TimeSpan.FromSeconds(300)).Tag("reports"));
});
...
app.MapUserActivityLogEndpoints();
